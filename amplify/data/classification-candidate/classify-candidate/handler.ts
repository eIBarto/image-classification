...
import { env } from "$amplify/env/classify-candidate";
...
export const handler: Schema["classifyCandidateProxy"]["functionHandler"] = async (event) => {
  const { identity } = event;
  const { classificationId, fileId } = event.arguments;
  ...
  const { data: classification, errors: classificationErrors } = await client.models.Classification.get({
    ...
    selectionSet: ["projectId", "promptId", "version", "temperature", "topP", "maxLength"]
  });
  ...
  const { projectId, promptId, version, temperature, topP, maxLength } = classification;
