...
export interface KrippendorffAlphaProps {
    projectId: string
    viewId: string
}

export function KrippendorffAlpha({ projectId, viewId }: KrippendorffAlphaProps) {
    ...
    useEffect(() => {
        if (error) {
            console.error(error)
            toast.error("Failed to get krippendorff alpha")
        }
    }, [error])
