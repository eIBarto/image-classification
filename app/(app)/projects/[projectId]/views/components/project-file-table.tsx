...
async function listFiles(options: Schema["listProjectFilesProxy"]["args"]) {
    const { data, errors } = await client.queries.listProjectFilesProxy(options)

    if (errors) {
        console.error(errors)
        throw new Error("Failed to fetch projects files")
    }
...
export function ProjectFileTable({ projectId, onSelect, value, isMulti = false }: ProjectFileTableProps) { // todo add nextToken
    ...
    */
    ...
    const {
        ...
        queryKey: ["ALL-project-files", projectId], // change key?
        queryFn: async ({
            pageParam,
        }: {
            ...
            const { items, nextToken = null } = await listFiles({ nextToken: pageParam, projectId: projectId, imageOptions: { width: 64, height: 64, format: "webp" } })
