...
export type EditViewFormSchema = z.infer<typeof formSchema>;

export interface EditViewFormProps extends Pick<React.ComponentProps<"form">, "className"> {
  onSubmit?: (values: EditViewFormSchema) => Promise<void | string> | void
  resetOnSuccess?: boolean
  disabled?: boolean
}

export function EditViewForm({ className, onSubmit, resetOnSuccess = true, ...props }: EditViewFormProps) {
  const form = useForm<EditViewFormSchema>({
  ...
  const handleSubmit = form.handleSubmit(async (values: EditViewFormSchema) => {
    try {
